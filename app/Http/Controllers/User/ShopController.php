...
class ShopController extends Controller
{
    ...
    public function customerOrders($orderCode){
        ...
        // Get the first order with this code to check payment status
        $orderInfo = Order::where('order_code', $orderCode)->first();
        
        return view ('user.userOrderDetails',compact('order', 'orderInfo'));
    }
    ...
    public function orderProduct(Request $request){
        ...
        $paymentMethod = Payment::find($request->paymentMethod);
        
        if ($paymentMethod && $paymentMethod->type === 'Paystack') {
            // For Paystack, validate without paySlipImage
            ...
            // For other payment methods, require paySlipImage
            $validator = $request->validate([
                'name' => 'required',
                'phone' => 'required',
                'paymentMethod' => 'required',
        ...
        // For other payment methods, save payment slip
        $data = [
